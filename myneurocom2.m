...
%     testTime(s)=D.TestTime;
    COP_x=double(D.mydata(:,21)); %% COGy in Neurocom AP in cm 
    COP_y=double(D.mydata(:,22)); %% COGx in Neurocom ML in cm   
%     COP_x=double(D.mydata(:,21).*10); %% COGy in Neurocom AP in mm 
%     COP_y=double(D.mydata(:,22).*10); %% COGx in Neurocom ML in mm
%     if isempty(COP_x)
